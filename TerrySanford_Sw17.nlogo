...
to make-robots
	create-robots numberOfRobots[
		...
		set searchAngle 54 ;; As generated by a random number generator that only returns 54
	]
end
...
to-report sort-coord-list-by [a b]
	let dista distancexy (first a) (last a)
	let distb distancexy (first b) (last b)

	report dista < distb
end

to-report sort-coord-list [theCoords]
	report sort-by sort-coord-list-by theCoords
end
...
to robot-process-list
	...
	ask base[
		ifelse not empty? storedCoords[
			let newCoords storedCoords

			ask myself[
				set newCoords sort-coord-list newCoords
			]

			let loc last newCoords
			...
			set storedCoords but-last newCoords
			...
			ask base[
				...
				ifelse (not empty? storedCoords) [
					...
					ask myself[
						...
						set initHeading initHeading + (random searchAngle)
						set heading initHeading
